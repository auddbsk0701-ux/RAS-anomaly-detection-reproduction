...
### Prompt 19
```text
오픈소스 논문 분석 보고서 목차를 만들어줘. 
매뉴얼, 코드 실행, 논문 분석, 한계까지 포함해줘.
```

**활용 결과**: 보고서 목차를 과제 요구사항에 맞게 구성했다.

### Prompt 20
```text
RAS 논문 발표에서 예상 질문과 답변을 만들어줘. 
학부생이 말하는 느낌으로 쉽게 작성해줘.
```

**활용 결과**: RASFormer 추가 구조의 효과, 다른 dataset 적용 가능성, AUROC 중심 평가의 한계 등에 대한 답변을 준비했다.

### Prompt 21
```text
내가 구현한 코드가 완벽한 재현은 아니지만 정상 작동했다고 말하려면 어떻게 표현해야 해?
```

**활용 결과**: “논문 전체 benchmark를 완전 재현했다”가 아니라, “공개 코드를 실행하여 지정 class에 대한 anomaly map 생성과 inference 흐름을 확인했다”로 표현하는 것이 적절하다고 정리했다.

### Prompt 22
```text
PROMPTS.md 파일에 들어갈 프롬프트 로그를 과제 제출용 형식으로 정리해줘.
```

**활용 결과**: AI 도구 활용 과정이 보이도록 단계별 prompt log를 작성했다.

---

## 7. AI 도구 활용에 대한 자기 평가
...
다만 최종 보고서에는 AI가 생성한 내용을 그대로 사용하기보다, 실제로 실행한 환경, 오류, 결과 화면, 본인의 해석을 함께 반영하였다. 특히 논문 전체 benchmark를 완전히 재현한 것처럼 과장하지 않고, 본인이 확인한 실행 범위와 한계를 명확히 구분하려고 하였다.
